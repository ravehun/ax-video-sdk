...
//         case BOX_svc1:   - no test
        ...
        case BOX_hvcC:
            ...
            {
                const boxsize_t hvcC_bytes = payload_bytes;
                MALLOC(unsigned char*, tr->dsi, (int)hvcC_bytes);
                for (i = 0; i < hvcC_bytes; i++)
                {
                    tr->dsi[i] = minimp4_read(mp4, 1, &eof_flag);
                }
                tr->dsi_bytes = (unsigned)hvcC_bytes;
                payload_bytes -= hvcC_bytes;
            }
            break;
#endif
