...
bool PrepareBitmap(const PipelineOsdBitmap& bitmap, PreparedBitmap* prepared_bitmap) {
    ...
    if ((row_bytes % 16U) != 0U) {
        std::fprintf(stderr, "pipeline osd ax650: bitmap row bytes must be 16-byte aligned, got %zu\n", row_bytes);
        return false;
    }
...
class PreparedAx650PipelineOsd final : public PreparedPipelineOsd {
public:
    explicit PreparedAx650PipelineOsd(std::uint32_t hold_frames) : hold_frames_(hold_frames) {}
    ...
    bool Apply(common::AxImage& image) const override {
        ...
        if (!bitmaps_.empty()) {
            ...
            if (AX_IVPS_DrawOsdTdp(&frame, attrs.data(), static_cast<AX_U32>(attrs.size())) != AX_SUCCESS) {
                return false;
            }
        }
...
class Ax650PipelineOsdRenderer final : public PipelineOsdRenderer {
public:
    std::shared_ptr<const PreparedPipelineOsd> Prepare(const PipelineOsdFrame& frame) override {
        auto prepared = std::make_shared<PreparedAx650PipelineOsd>(frame.hold_frames);
...
std::unique_ptr<PipelineOsdRenderer> CreatePlatformPipelineOsdRenderer() {
    return std::make_unique<Ax650PipelineOsdRenderer>();
}
